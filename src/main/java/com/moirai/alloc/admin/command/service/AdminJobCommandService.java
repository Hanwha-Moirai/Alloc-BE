...
public class AdminJobCommandService {
    ...
    public Long createJob(String jobName) {
        String normalized = normalizeJobName(jobName);
        if (jobStandardRepository.existsByJobNameIgnoreCase(normalized)) {
    ...
    public Long updateJob(Long jobId, String jobName) {
        ...
        if (jobStandardRepository.existsByJobNameIgnoreCase(normalized)
                && !jobStandard.getJobName().equalsIgnoreCase(normalized)) {
            throw new IllegalArgumentException("이미 존재하는 직무입니다.");
        }
        jobStandard.updateJobName(normalized);
        return jobStandard.getJobId();
    }
    ...
    private String normalizeJobName(String jobName) {
        if (jobName == null) {
            throw new IllegalArgumentException("직무 이름이 필요합니다.");
        }

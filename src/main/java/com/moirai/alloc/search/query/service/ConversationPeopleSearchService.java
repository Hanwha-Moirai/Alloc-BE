...
import com.moirai.alloc.search.query.service.policy.ConversationSearchFlowPolicy;
...
public class ConversationPeopleSearchService {
    ...
    private final ConversationSearchFlowPolicy flowPolicy;
    ...
    public ConversationSearchResponse search(String nl) {
        //자연어-> 구조화된 intent(gpt)
        SearchIntent intent = searchIntentParser.parse(nl);

        ClarificationType clarification = flowPolicy.decide(intent);

        if (clarification != null) {
            return ConversationSearchResponse.question(clarification);
        }
        ...
        if (result.isEmpty() && lastResult != null) {
            return ConversationSearchResponse.result(
                    personViewMapper.toViews(lastResult)
            );
        }
        ...
        return ConversationSearchResponse.result(
                personViewMapper.toViews(result)
        );
    }
    private SearchCondition toCondition(SearchIntent intent) {
        return SearchCondition.of(
                intent.getFreeText(),
                intent.getSkillConditions() != null ? intent.getSkillConditions() : List.of(),
